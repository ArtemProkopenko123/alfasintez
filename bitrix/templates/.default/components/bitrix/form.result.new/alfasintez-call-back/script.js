$( document ).ready(function() {
	...
	$('.alfasintez-call-back-form .alfasintez-question-1 input').attr({placeholder: "Имя *",data_only_text:""});
	$('.alfasintez-call-back-form .alfasintez-question-2 input').attr({placeholder: "Номер телефона *",phone:""});
...
// проверка формы на заполненность////////
    $('body').on('click', '.alfasintez-call-back-form .alfasintez-form-submit', function(event)
    {
        ...
        var $input_1 = $('.alfasintez-call-back-form .alfasintez-question-1 input'),
            $input_2 = $('.alfasintez-call-back-form .alfasintez-question-2 input');

        if($input_1.val() == "" ||  $input_1.val().length < 3 ) {
            $input_1.attr("placeholder", "Имя *").addClass("inputError");
            event.preventDefault();
        } else {$(this).attr("disable", true);}

        if($input_2.val() == "" || $input_2.val().length < 10 || $input_2.val().length > 15) {
            $input_2.attr("placeholder", "Номер телефона *").addClass("inputError");
            event.preventDefault();
        } else {$(this).attr("disable", true);}
